...
# --- Custom Table Widget with Paste/Delete Support ---
class PasteDeleteTableWidget(QTableWidget):
    """QTableWidget with copy-paste-delete support for multi-cell operations.
    
    Special handling for time_interval column (column 3):
    - When pasting multi-column data, only the first column is retained
    - When pasting more rows than available, excess rows are discarded
    """
    ...
    def delete_selected_cells(self):
        """Clear contents of all selected cells (except read-only cells)."""
    ...
    def paste_from_clipboard(self):
        ...
        For time_interval column (column 3): only first column of pasted data is used.
        Rows beyond table bounds are discarded.
        """
        ...
        for i, row_text in enumerate(rows):
            ...
            if target_row >= self.rowCount():
                break  # Discard rows beyond table bounds
            
            # Split by tab (Excel/spreadsheet format)
            cells = row_text.split('\t')
            
            # Special handling: if pasting into time_interval column (col 3),
            # only use the first column of pasted data
            if start_col == 3:
                # Only paste the first value into the time_interval column
                if cells:
                    cell_text = cells[0].strip()
                    item = self.item(target_row, start_col)
                    if item is None:
                        item = QTableWidgetItem()
                        self.setItem(target_row, start_col, item)
                    if item.flags() & Qt.ItemIsEditable:
                        item.setText(cell_text)
            else:
                # Normal paste for other columns
...
class FolderOverridesDialog(QDialog):
    ...
    def __init__(self, base_paths: List[str], default_time_interval: float, parent=None):
        ...
        info = QLabel(
            ...
            "Tip: Use Ctrl+V to paste, Delete/Backspace to clear cells."
        )
        layout.addWidget(info)

        # Use custom table widget with paste/delete support
        self.table = PasteDeleteTableWidget(len(self.base_paths), 5)
