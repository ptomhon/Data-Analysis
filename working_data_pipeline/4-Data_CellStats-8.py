...
class DataStatsGUI(QWidget):
    def __init__(self):
        ...
        # Initial window size is now set in main() via INITIAL_WINDOW_W/H
...
# ===== Entrypoint =====
# ----- Hardcoded UI scaling -----
# Everything below controls how small/large the GUI looks. Edit these numbers
# and re-run; no env vars, no config files. Smaller font = smaller everything
# (buttons, tables, labels resize around the font metric).
UI_FONT_POINT_SIZE = 7        # default Qt is ~9-10. Try 7 for very small.
INITIAL_WINDOW_W = 900        # was 1100 in v7
INITIAL_WINDOW_H = 720        # was  900 in v7
MATPLOTLIB_FONT_SIZE = 7      # font on plots (axes, ticks, legend, titles)


def main():
    app = QApplication(sys.argv)

    # Apply a smaller application-wide font. This is the single biggest lever
    # for making the GUI compact without disabling Qt's DPI handling (which
    # can cause clipping). All widgets that don't override the font inherit
    # this one, so tables, buttons, labels, group boxes all shrink together.
    font = app.font()
    font.setPointSize(UI_FONT_POINT_SIZE)
    app.setFont(font)

    # Match matplotlib's default font size to the UI so embedded plots feel
    # consistent in density with the surrounding widgets.
    plt.rcParams.update({
    ...
    window = DataStatsGUI()
    window.resize(INITIAL_WINDOW_W, INITIAL_WINDOW_H)
    window.show()
    sys.exit(app.exec_())
