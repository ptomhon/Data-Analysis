...
def read_pd_fid_strict(path):
    ...
    # time may be string/complex-like — coerce safely to float seconds
    try:
...
def hz_to_ppm(freq_hz: np.ndarray, larmor_hz: float) -> np.ndarray:
    return 1e6 * (freq_hz / float(larmor_hz))
...
class Processor:
    ...
    def process(self, fid, time, extra_phase_offset=0.0):
        ...
        # --- Axis in ppm (carrier mode, 0 Hz → 0 ppm) ---
        ppm = hz_to_ppm(freq_hz, self.larmor_hz) if self.use_ppm else freq_hz
        ppm_min, ppm_max = (float(np.nanmin(ppm)), float(np.nanmax(ppm))) if ppm.size else (np.nan, np.nan)
        # --- Integration window (order-agnostic) ---
        lo, hi = sorted([self.ppm_stop, self.ppm_start]) if self.use_ppm else sorted([self.ppm_stop, self.ppm_start])
        mask = (ppm >= lo) & (ppm <= hi)
        if np.any(mask):
            x = ppm[mask]
            y = real_bc[mask]
        ...
        overlay = f"Int: {integral:.3f}\nRange: {lo:.2f}–{hi:.2f} {'ppm' if self.use_ppm else 'Hz'}"
        ax.text(0.98, 0.02, overlay, transform=ax.transAxes,
                ha='right', va='bottom', fontsize=11,
                bbox=dict(facecolor='white', alpha=0.8, edgecolor='black'))
        # Also show ppm span for debugging integration range
        span_txt = f"Span: {ppm_min:.1f}–{ppm_max:.1f} ppm"
        ax.text(0.02, 0.02, span_txt, transform=ax.transAxes,
