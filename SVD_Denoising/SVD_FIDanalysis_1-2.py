...
def process_single_folder(
    ...
    """
    Single-folder processing (now mirrors the multiple-folder pipeline):
      1) Load FID
      2) Denoise via Hankel-SVD with quality-check loop over k
      3) Zero-fill, exponential apodization, phase correction
      4) FFT -> peak finding -> baseline-based integration windows
      5) Match to target peaks (heights & integrals)
      6) Optional plotting (time-domain + FFT)

    Returns:
        dict with:
          - 'folder', 'file_path', 'optimal_k'
          - 'new_time', 'noisy_processed', 'denoised_processed'
          - 'ppm_axis', 'signal_real'
          - 'peaks' (indices), 'peak_windows' (list of (L,R) indices)
          - 'peak_data' (list of dicts with position/height/integral/edges)
          - 'target_heights', 'target_integrals'
          - 'time_point' (if time_interval provided)
    """
    import os, time
    import numpy as np
    from scipy.signal import find_peaks
    ...
    # Defaults to mirror multi-folder behavior
    ...
    # ---- Helpers copied from multi-folder workflow ----
    ...
    # ---- 2) Denoise with k loop + FFT quality check (identical logic) ----
    n_denoise = 2 * L - 1
    ...
    # ---- 3) Zero-fill, apodize, phase-correct (same as multi-folder) ----
    ...
    # ---- 4) FFT & baseline-based integration windows (copied logic) ----
    ...
    peak_data = []
    if len(peaks) > 0:
        # Initial windows from baseline method
        for p in peaks:
            L_edge = baseline_left_edge(signal_real, p, fraction=0.05)
            R_edge = baseline_right_edge(signal_real, p, fraction=0.05)
            peak_windows.append((L_edge, R_edge))

        # Resolve overlaps by splitting at the minimum
        ...
        # Compute integrals for each detected peak
...
process_single_folder(folder=58, base_path=data_c,
